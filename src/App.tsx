        ...
        {showWarning && (
          ...
          >
            <motion.div
              ...
              className="relative bg-white rounded-[2rem] shadow-2xl max-w-md w-full p-8 md:p-10 border border-slate-100 flex flex-col items-center text-center"
            >
              {/* Botão de Fechar */}
              <button 
                onClick={() => setShowWarning(false)}
                className="absolute top-6 right-6 p-2 text-slate-400 hover:text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-full transition-colors focus:outline-none"
              ...
              {/* Conteúdo */}
              <span className="text-xs font-bold text-amber-600 uppercase tracking-widest mb-2 block">
                Comunicado Importante
              </span>
              
              <h3 className="text-2xl md:text-3xl font-black text-rose-600 mb-4 tracking-tight leading-tight">
                NÃO VENDEMOS PEÇAS!
              </h3>

              <p className="text-slate-600 leading-relaxed text-sm md:text-base mb-8">
                Prestamos exclusivamente serviços profissionais de <strong>conserto, manutenção e assistência técnica em domicílio</strong> para geladeiras, lavadoras e lava-louças.
              </p>

              {/* Botão Principal */}
              <button
                onClick={() => setShowWarning(false)}
                className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all shadow-lg shadow-blue-200 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] focus:outline-none"
              >
                Entendido, ver serviços
              </button>
            </motion.div>
          </motion.div>
        )}
